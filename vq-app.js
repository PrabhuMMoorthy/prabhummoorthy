...
function executeSearch() {
  ...
  data.books.forEach(function(b) {
    ...
    b.chapters && b.chapters.forEach(function(ch) {
      ...
      // Search inside philosophical_quotes
      ch.philosophical_quotes && ch.philosophical_quotes.forEach(function(q) {
        var text = (q.quote || '').toLowerCase();
        var charName = (q.character_name || '').toLowerCase();
...
function renderSearchResults() {
  ...
  // Calculate slices for pagination (100 per page)
  ...
  // Set the context quotes global so export image button works for currently showing set
  ...
  // Append pagination controls if multiple pages exist
  if (totalPages > 1) {
  ...
  // Smooth scroll back to top of content area
  c.scrollTop = 0;
}
